...
export async function geminiGenerateTweetSuggestions(context: TweetContext, draft?: string): Promise<string> {
  ...
  const prompt = `
You are assisting Superteam Vietnam with managing their Twitter account. Based on the provided context, propose engaging tweets or refine a draft tweet, also correct misspelt handles from the draft, using the list of followed accounts given.
